...
(function () {
  ...
  function optionButtons(kind, items) {
  ...
  function enhancePaymentMethods() {
    ...
    box.innerHTML = [
      ...
      '<div class="payment-method-group"><button type="button" class="pay-method payment-group" onclick="togglePaymentGroup(this)">' + LABELS.credit + ' ▾</button><div class="payment-submethods">' + optionButtons("credit", [
        { value: PAYMENT_METHODS.credit_bca, label: "🏦 BCA" },
        { value: PAYMENT_METHODS.credit_mandiri, label: "💳 Mandiri" }
      ]) + '</div></div>',
      '<div class="payment-method-group"><button type="button" class="pay-method payment-group" onclick="togglePaymentGroup(this)">' + LABELS.debit + ' ▾</button><div class="payment-submethods">' + optionButtons("debit", [
        { value: PAYMENT_METHODS.debit_bca, label: "🏦 BCA" },
        { value: PAYMENT_METHODS.debit_mandiri, label: "💳 Mandiri" }
      ]) + '</div></div>',
      '<div class="payment-method-group"><button type="button" class="pay-method payment-group" onclick="togglePaymentGroup(this)">' + LABELS.wallet + ' ▾</button><div class="payment-submethods">' + optionButtons("wallet", [
        { value: PAYMENT_METHODS.wallet_gopay, label: "GoPay" },
        { value: PAYMENT_METHODS.wallet_ovo, label: "OVO" },
        { value: PAYMENT_METHODS.wallet_dana, label: "DANA" },
        { value: PAYMENT_METHODS.wallet_shopeepay, label: "ShopeePay" },
        { value: PAYMENT_METHODS.wallet_linkaja, label: "LinkAja" }
      ]) + '</div></div>',
      '<div class="payment-method-group"><button type="button" class="pay-method payment-group" onclick="togglePaymentGroup(this)">' + LABELS.bank + ' ▾</button><div class="payment-submethods">' + optionButtons("bank", [
        { value: PAYMENT_METHODS.bank_bca, label: "🏦 BCA" },
        { value: PAYMENT_METHODS.bank_mandiri, label: "💳 Mandiri" },
        { value: PAYMENT_METHODS.bank_other, label: "🏦 بنك آخر" }
      ]) + '</div></div>'
    ].join("");
  ...
  window.togglePaymentGroup = function (button) {
    ...
    if (!group) return;
    document.querySelectorAll("#pay-methods .payment-method-group.open").forEach(function (g) {
  ...
  if (typeof window.setPayMethod === "function") {
    ...
    window.setPayMethod = function (el) {
      ...
      document.querySelectorAll("#pay-methods .payment-method-group").forEach(function (g) {
  ...
  if (typeof window.showPayment === "function") {
    ...
    window.showPayment = function () {
      originalShowPayment.apply(this, arguments);
      enhancePaymentMethods();
    };
  }

  Object.assign(window, {
    __enhancePaymentMethods: enhancePaymentMethods
  });
  ...
  style.textContent = `
    ...
    @media (max-width:520px) {
      #pay-methods .payment-submethods { grid-template-columns:1fr; }
    }
  `;
